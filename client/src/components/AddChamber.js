...
export default function AddChamber({id,add,setAdd}){
    const {addDoctor} = useUserStore()
    ...
    async function addChamber(){
        const res = await axios.post(`http://localhost:8080/api/doctor/addChamber/${id}`,value,{
            headers : {
