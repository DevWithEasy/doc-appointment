    ...
    return(
        ...
        >
            ...
            <div className='py-2 space-y-1'>
                {
                    notifications && !readState && notifications.filter(notification=>notification.status === 'unread').map(notification=><Link to={notification?.onClickPath} key={notification.id} onClick={()=>seenNotification(notification,addUser)} className='block w-full p-1 border rounded'>
